...
export default class Example extends Component {
    ...
    myfollow(user) {
        axios('/user/follow/' + this.state.user.id, {method: "POST"}).then(response => {
    ...
    componentDidMount() {
        ...
        if (followed === true) {
            this.setState({btnText: 'Following', className: 'following-button'});
        }
    }
    ...
    btnClick() {
        ...
        if (this.state.className === 'follow-button') {
            this.setState({className: 'following-button'})
        } else {
            this.setState({className: 'follow-button'})
        }
...
if (document.getElementById('profile')) {
    var data = document
        ...
        .getAttribute('data');
    ReactDOM.render(
        <Example data={data}/>, document.getElementById('profile'));
}
